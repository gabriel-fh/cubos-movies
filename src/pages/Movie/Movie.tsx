    ...
    <main className="relative p-4 xl:p-8">
      <section className="w-full relative grid grid-cols-1 gap-4 md:px-2 md:py-4 xl:p-8 md:grid-cols-2 
        ...
        <div
          ...
          style={{ backgroundImage: data && `url(${IMG_BASE_URL}${data.backdrop_path})` }}
        />
        {isLoading ? (
          <Skeleton />
        ) : !data ? (
          <NoResults variant="secondary" className="col-span-full min-h-screen" />
        ) : (
          <>
      ...
      {data && getTrailerId(data?.videos?.results) &&
        <section className="mt-4 md:px-2 xl:px-8">
          <h2 className="text-mauve12 text-2xl xl:text-3xl font-semibold mb-2">Trailer</h2>
          <iframe
            title="Trailer"
            className="w-full aspect-video rounded-sm"
            src={`https://www.youtube.com/embed/${getTrailerId(data.videos.results)}`}
