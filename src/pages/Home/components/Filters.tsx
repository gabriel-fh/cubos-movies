...
const Filters = ({ genresData }: FiltersProps) => {
  ...
  return (
    <Drawer.Portal>
      ...
      <Drawer.Content className="bg-mauve8 flex flex-col rounded-t-[10px] mt-24 h-screen fixed bottom-0 left-0 right-0 outline-none">
              ...
              <div>
                ...
                <ul className="flex flex-wrap gap-2 mt-2">
                  {genresData.genres.map((genre) => (
                    <li key={genre.id}>
                      <Button className="text-xs text-mauve12 px-3 py-[3px] rounded-full capitalize border border-purple3"
                        variant="secondary"
                      >
                        {genre.name}
                      </Button>
